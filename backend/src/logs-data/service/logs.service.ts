...
export class LogsService {
  ...
  //   total events
  async getTotalEvents({ source }: SourceInput): Promise<TotalEventsResponse> {
    const match = getSourceFilter(source as SOURCE);
  ...
  async getMostActiveIpAddress({
    source,
  }: SourceInput): Promise<ActiveIPResponse> {
    const match = getSourceFilter(source as SOURCE);
  ...
  //   most active ip address
  async getMostCommonMethod({ source }: SourceInput) {
    const match = getSourceFilter(source as SOURCE);
  ...
  async getTopHttpStatusCode({ source }: SourceInput) {
    const match = getSourceFilter(source as SOURCE);
  ...
  async mostCommonResponseSize({ source }: SourceInput) {
    const match = getSourceFilter(source as SOURCE);
  ...
  async getTopUserAgent({ source }: SourceInput) {
    const match = getSourceFilter(source as SOURCE);
  ...
  async getStat({ source }: SourceInput) {
    ...
    const [
      ...
      timeout(this.getTotalEvents({ source }), maxQueryTime),
      timeout(this.getMostActiveIpAddress({ source }), maxQueryTime),
      timeout(this.getMostCommonMethod({ source }), maxQueryTime),
      timeout(this.getTopHttpStatusCode({ source }), maxQueryTime),
      timeout(this.mostCommonResponseSize({ source }), maxQueryTime),
      timeout(this.getTopUserAgent({ source }), maxQueryTime),
    ]);
